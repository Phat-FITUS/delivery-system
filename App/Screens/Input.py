...
class InputScreen(Screen):
    ...
    def run(self) -> None:
        while self.running:
            self.drawBackground()
            self.displayText("CHOOSE INPUT", self.font, Color.WHITE, self.SCREEN_WIDTH // 2, 100)

            self.btn_inp1.draw(self.screen, self.SCREEN_WIDTH // 2, 170, self.handleInput(1))
            self.btn_inp2.draw(self.screen, self.SCREEN_WIDTH // 2, 240, self.handleInput(2))
            self.btn_inp3.draw(self.screen, self.SCREEN_WIDTH // 2, 310, self.handleInput(3))
            self.btn_inp4.draw(self.screen, self.SCREEN_WIDTH // 2, 380, self.handleInput(4))
            self.btn_inp5.draw(self.screen, self.SCREEN_WIDTH // 2, 450, self.handleInput(5))
